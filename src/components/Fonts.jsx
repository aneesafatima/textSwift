...
function Fonts() {
  ...
  useEffect(() => {
    const generateFonts = async () => {
      const response = await fetch(
        `https://www.googleapis.com/webfonts/v1/webfonts?key=${process.env.REACT_APP_FONTS_API_KEY}`
      );
  ...
  return (
    <select
      className="dropdown-fontFamily"
      onChange={(e) => handleClick(e, "fontFamily")}
    >
      ...
      {fontsLoaded &&
        fontsNames?.map((el, i) => {
          if (
            c < 10 &&
            (el.startsWith(char.toLowerCase()) || el.startsWith(char))
          ) {
            c++;
            return (
              <option
